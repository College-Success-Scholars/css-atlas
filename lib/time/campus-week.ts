 ...
 * All campus calendar boundaries use this zone — not the server or browser default.
 */
...
/** Calendar year / month (0-based) / day in US Eastern for the instant `d`. */
...
/** Whole calendar days from earlier → later (Eastern start-of-day to Eastern start-of-day). */
...
/** Monday midnight US Eastern of the week containing the given date. */
...
/** First Monday on or after the day after winter break (US Eastern; start of first spring week) */
...
export type CampusWeekDateRange = {
  ...
  weekNumber: number;
  /** Start of the week (inclusive), midnight US Eastern ({@link EASTERN_TIMEZONE}) */
  startDate: Date;
  /** End of the week (inclusive), midnight US Eastern of the last calendar day */
  endDate: Date;
};
 ...
 *
 * @param date - Any Date (typically created_at from DB); interpreted in US Eastern ({@link EASTERN_TIMEZONE})
