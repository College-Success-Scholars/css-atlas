...
export function getObjectValueByKeyPattern(
...
/** Hours suffix for display (e.g. "1 hr", "4.5 hrs"). */
...
export function parseWplProjectRows(projects: unknown): WplProjectRow[] {
  ...
  for (const item of projects) {
    ...
    if (typeof item === "string") {
      const s = item.trim()
      if (s) rows.push({ name: s, hours: "" })
      continue
    }
    if (typeof item !== "object") {
      rows.push({ name: String(item), hours: "" })
      continue
    }
    const obj = item as Record<string, unknown>
    const projectName = getObjectValueByKeyPattern(obj, [
...
const GRADE_NUMERIC = /(\d+(?:\.\d+)?)/

export function parseNumericGrade(grade: string): number | null {
  const m = grade.trim().match(GRADE_NUMERIC)
  if (!m) return null
  const n = Number.parseFloat(m[1]!)
  return Number.isFinite(n) ? n : null
}

/** Tailwind text color classes for numeric grades (light + dark). */
...
/** "None" when no / negative; short label when missed. */
